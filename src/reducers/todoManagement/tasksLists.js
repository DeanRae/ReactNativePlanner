...
const taskLists = (state = initialState, action) => {
    switch (action.type) {
        case types.LISTS_FETCHED:
            return { ...state, taskLists: action.lists, taskOperationLoading: false };
        case types.LIST_DELETION_SUCCESS:
            ...
            return { ...state, taskLists: { byId: newById, allIds: initialState.taskLists.allIds.filter(list => list !== action.listId) }, listOperationLoading: false };
